...
def proxy_args() -> dict:
    """Возвращает kwargs для curl_cffi.AsyncSession с прокси (если задан PROXY_URL).

    Формат curl_cffi: proxies={"http": "...", "https": "..."}.
    Поддерживает http://, https://, socks5://.
    Если PROXY_URL не задан — возвращает пустой dict (прямое соединение).
    """
    if not _PROXY_NORMALIZED:
        return {}
    return {"proxies": {"http": _PROXY_NORMALIZED, "https": _PROXY_NORMALIZED}}
...
def make_client(
    *,
    ...
    ios: bool = False,
    timeout: float = 20.0,
    http2: bool = False,  # игнорируется — impersonate сам управляет ALPN/h2
    extra_headers: dict | None = None,
) -> AsyncSession:
    """Фабрика изолированной curl_cffi.AsyncSession.

    Имитирует Chrome 124 на уровне TLS/JA3 + HTTP/2 fingerprint —
    обходит TLS Fingerprinting (Facebook, LinkedIn, CryptoRank).

    Каждая сессия получает:
      - impersonate="chrome124" (TLS + h2 fingerprint Chrome 124)
      - случайный User-Agent из пула (поверх impersonate)
      - прокси из PROXY_URL (если задан)
      - изолированный cookie jar (на уровне сессии)
      - реалистичные browser-заголовки
    """
    hdrs = browser_headers(mobile=mobile, ios=ios)
    if extra_headers:
        hdrs.update(extra_headers)
    _ = http2  # сохранён в сигнатуре для обратной совместимости
    return AsyncSession(
        headers=hdrs,
        timeout=timeout,
...
async def solve_recaptcha_v2(site_key: str, page_url: str) -> str | None:
    ...
    if not CAPTCHA_KEY:
        return None
    try:
        async with AsyncSession(timeout=30.0, impersonate="chrome124", **proxy_args()) as c:
            r = await c.post(
